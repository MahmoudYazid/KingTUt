    ...
    <div className="min-h-screen min-w-100 flex   justify-center items-center 	bg-white		"  >
      {
        IdData.map((GetData)=>(
          <div className='flex md:flex-col flex-col w-[60rem] min-h-[30rem] bg-white   m-3 text-center border-black border-2  '>
            <img src={GetData.imgname == '' ? path.join(process.cwd(), `/public/Pharaoh.svg`) : path.join(process.cwd(), `/public/UploadedImg/${GetData.imgname}`) } className='min-w-[10rem] h-[10rem]  self-center ml-5	mt-5 mb-5 hover:scale-[2]	'></img>
